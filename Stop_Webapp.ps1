...
function authAzure {
    ...
    StopWebApps $WebAppConfigurations
}

function StopWebApps($WebAppConfigurations) {
    foreach ($resourceGroup in $WebAppConfigurations.Keys) {
        ...
        foreach ($webAppName in $webApps) {
            ...
            if ($webApp.State -eq "Stopped") {
                Write-Output "Web App $webAppName is already stopped"
            }
            else {
                Stop-AzWebApp -ResourceGroupName $resourceGroupName -Name $webAppName
                Write-Output "Stopped Web App $webAppName in resource group $resourceGroupName"
            }
        }
    }
}
